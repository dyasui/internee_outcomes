...
#+begin_src R :colnames yes :tangle no :session *R:internment*
library(tidyverse)
...
These are preliminary for now, but mostly correspond to 10-15 year time spans.
The longer time spans are trying to account for a seeming tendency of older census respondents to round their birht year to the nearest decade or multiple of 5.

#+begin_src R :tangle R/helper_functions.R :session *R:internment*
...
#+begin_src R
# counties in the evacuation zone by ICP codes
...
predict_internment <- function(data, census_1940, by = c("STATEFIP", "COUNTYICP")) {
  library(tidyverse)
  library(dbplyr)
  ...
  # add cohort variable to census
   census_1940 <- census_1940 |>
    rename_with(tolower) |>
    mutate(cohort = as_cohort(birthyr) )
