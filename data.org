...
* Sample Selection
After linking individual census records by HIKs across the 1940 and 1950 sample, there are discrepancies in reported races.
#+begin_src R
tar_load(mlp_sample)
mlp_sample |>
  select(YEAR, RACE, id) |>
...
** PROJ Reading IPUMS data into database
...
I want to gather overall summary statistics on Japanese and Chinese populations from the full-count 1940 census.
There isn't enough local memory available to hold all observations and variables at one time to calculate.
To deal with this, I could either:
  a) read data from the =usa_*****.dat.gz= file in chunks and estimate population level statistics from chunks
  b) use =duckdb= to read the full-count =usa_*****.dat.gz= file into a new table in the database and then estimate using =dbplyr= tools

*** a) Chunking with =read_ipums_micro_chunked=
I ran out of memory after the function tried to go from chunks to the overall summary tibble.
Maybe there are too many variables to do this all at once with.
Another problem with this is that there's not an easy way to get a total variance back from chunked summaries of means and variances.

#+begin_src R :tangle nil
sumstats_census <- function(ddi,
                            internment_groups,
                            int_grps = c("STATEFIP", "COUNTYICP",
                                         "RACE", "SEX", "BIRTHYR", "BPL")) {
  ...
  summary_cb <- function(x, pos) {
    x |>
      ...
      mutate(
        byr_clean = ifelse(BIRTHYR %in% 9996:9999, NA, BIRTHYR),
        ## iyr_clean = ifelse(YRIMMIG==0, NA, YRIMMIG),
      ) |>
      na_dollar_vals() |>
      filter(if_all(c("byr_clean", "INCWAGE"), ~ !is.na(.))) |>
      group_by(across(all_of(int_grps))) |>
      summarise(
        n_chunk = n(),
        Male = sum(SEX==1),
        FirstGen = sum(NATIVITY==5),
        mean_Age = 1942 - mean(byr_clean, na.rm=T),
        var_Age = var(byr_clean, na.rm=T),
        ## mean_ImmigYr = mean(iyr_clean, na.rm=T),
        ## var_ImmigYr = var(iyr_clean, na.rm=T),
        Employed = sum(EMPSTAT==1),
        Unemployed = sum(EMPSTAT==2),
        NotinLF = sum(EMPSTAT==3),
        mean_Wage = mean(INCWAGE),
        var_Wage = var(INCWAGE),
        mean_Occscore = mean(OCCSCORE, na.rm=T),
        var_Occscore = var(OCCSCORE, na.rm=T),
        EvacZone = sum(is_evac_county(STATEFIP, COUNTYICP)),
      )
  }

  fullcount_sum_tbl <- read_ipums_micro_chunked(ddi,
                                                callback = IpumsDataFrameCallback$new(summary_cb),
                                                chunk_size = 1e7,
                                                vars = c("YEAR", "STATEFIP", "COUNTYICP",
                                                         "RACE", "SEX" , "BIRTHYR" , "BPL",
                                                         "NATIVITY", "EMPSTAT",
                                                         "INCWAGE", "OCCSCORE"),
                                                verbose = TRUE) |>
    ...
    summarise(
      n = sum(n_chunk),
      across(any_of(c("Male", "FirstGen", "EvacZone", "Employed", "NotinLF")),
             ~ sum(.x)/sum(n_chunk)),

      # Overall Means
      mean_Age = sum(n_chunk * mean_Age) / sum(n_chunk),
      mean_ImmigYr = sum(n_chunk * mean_ImmigYr) / sum(n_chunk),
      mean_Wage = sum(n_chunk * mean_Wage) / sum(n_chunk),
      mean_Occscore = sum(n_chunk * mean_Occscore) / sum(n_chunk),

      # Overall Variances using the parallel formula
      var_Age = sum((n_chunk - 1) * var_Age + n_chunk * (mean_Age - sum(n_chunk * mean_Age)/sum(n_chunk))^2) / (sum(n_chunk) - 1),
      var_ImmigYr = sum((n_chunk - 1) * var_ImmigYr + n_chunk * (mean_ImmigYr - sum(n_chunk * mean_ImmigYr)/sum(n_chunk))^2) / (sum(n_chunk) - 1),
      var_Wage = sum((n_chunk - 1) * var_Wage + n_chunk * (mean_Wage - sum(n_chunk * mean_Wage)/sum(n_chunk))^2) / (sum(n_chunk) - 1),
      var_Occscore = sum((n_chunk - 1) * var_Occscore + n_chunk * (mean_Occscore - sum(n_chunk * mean_Occscore)/sum(n_chunk))^2) / (sum(n_chunk) - 1),
      .groups = "drop"
    )

  return(fullcount_sum_tbl)
}

#+end_src

#+RESULTS:
**** Trying one variable at a time
Maybe a better way to do this is to write a function that creates a population level summary for one variable at a time and handle counts differently than average floats.
#+begin_src R :results silent
library(tidyverse)
library(targets)
library(ipumsr)
...
chunks |>
  ...
  predict_internment(proportions = internment_groups,  by = group_by) |>
  group_by(RACE, intern_status) |>
  summarise(
    n = sum(n_chunk),
    across(ends_with("_mean"), ~ sum(.x * n_chunk, na.rm = TRUE) / sum(n_chunk)),
    across(ends_with("_sd"), ~ sum((n_chunk - 1) * .x + n_chunk * (.x - sum(n_chunk * .x)/sum(n_chunk))^2, na.rm = TRUE) / (sum(n_chunk) - 1) )
  )
#+end_src

#+RESULTS:
| RACE | intern_status |      n |     INCWAGE_mean |    OCCSCORE_mean | INCWAGE_sd | OCCSCORE_sd |
|------+---------------+--------+------------------+------------------+------------+-------------|
|    4 | FALSE         | 165408 | 470.218038561939 | 10.7900766589282 |          0 |           0 |
|    4 |               |  32319 | 3.20424395829357 | 10.5397444227854 |          0 |           0 |
|    5 | FALSE         |  48677 | 738.067566567338 | 9.32717710623087 |          0 |           0 |
|    5 | TRUE          | 356408 | 179.131245729406 | 8.54728288927297 |          0 |           0 |
|    5 |               |    219 | 874.375365882215 | 5.89041095890411 |          0 |           0 |

*** b) Reading from a =duckdb= table
Again, I tried this with all of the variables at once, but ran out of memory.
#+begin_src R :tangle nil
sumstats_census <- function(db, tbl,
                            internment_groups,
                            int_grps = c("STATEFIP", "COUNTYICP",
                                         "RACE", "SEX", "BIRTHYR", "BPL"),
                            vars = c("NATIVITY", "EMPSTAT",
                                     "OCCSCORE", "INCWAGE", "FARM")) {
  con <- dbConnect(duckdb(), dbdir = db)

  group_data <- tbl(con, tbl) |>
    filter(YEAR==1940) |>
    select(YEAR, all_of(int_grps), all_of(vars)) |>
    mutate(
      BIRTHYR = ifelse(BIRTHYR %in% 9996:9999, NA, BIRTHYR),
      ## iyr_clean = ifelse(YRIMMIG==0, NA, YRIMMIG),
      ) |>
    na_dollar_vals() |>
    filter(if_all(c("BIRTHYR", "INCWAGE", "STATEFIP"), ~ !is.na(.))) |>
    group_by(across(all_of(int_grps))) |>
    summarise(
      n_grp = n(),
      Male = sum(as.integer(SEX==1)),
      FirstGen = sum(as.integer(NATIVITY==5)),
      Farm = sum(as.integer(FARM==2)),
      mean_Age = 1942 - mean(BIRTHYR, na.rm=T),
      sd_Age = sd(BIRTHYR, na.rm=T),
      ## mean_ImmigYr = mean(iyr_clean, na.rm=T),
      ## sd_ImmigYr = sd(iyr_clean, na.rm=T),
      Employed = sum(as.integer(EMPSTAT==1)),
      Unemployed = sum(as.integer(EMPSTAT==2)),
      NotinLF = sum(as.integer(EMPSTAT==3)),
      mean_Wage = mean(INCWAGE),
      sd_Wage = sd(INCWAGE),
      mean_Occscore = mean(OCCSCORE, na.rm=T),
      sd_Occscore = sd(OCCSCORE, na.rm=T),
      ) |>
    collect()

  sum_stats <- group_data |>
    predict_internment(proportions = internment_groups,  by = int_grps) |>
    mutate(EvacZone - (is_evac_county(STATEFIP, COUNTYICP))) |>
    ungroup() |>
    group_by(RACE, intern_status) |>
    summarise(
      n = sum(n_grp),
      Male = sum(Male)/sum(n_grp),
      FirstGen = sum(FirstGen)/sum(n_grp),
      Farm = sum(Farm)/sum(n_grp),
      Age = sum(mean_Age*n_grp)/
    )
...
* Geography
** WRA internee previous addresses
...
***
** West Coast internment proportions
#+begin_src R :tangle R/mapping.R
...
* IDEA Japanese American Redress Verification Information System (JARVIS) Files
:PROPERTIES:
...
* IDEA Japanese American Research Project (JARP)
