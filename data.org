...
** IPUMS API Download
:PROPERTIES:
:header-args:R: :session *R:ipums* :colnames yes :export no
:END:
#+name: load-packages
#+begin_src R :results silent
library(ipumsr)
...
tbl = "ipums_microdata"
linking_vars <- c("STATEFIP", "COUNTYICP", "SEX", "RACE")
...
mlp_panel_linked <- mlp_panel |>
  ...
  mutate(
    ...
    treat = ifelse(p_ij>0.5, "Interned", "Non-interned"),
    group = case_when(RACE==4 ~ "Chinese",
                      RACE==5 & treat=="Interned" ~ "Interned Japanese",
                      RACE==5 & treat=="Non-interned" ~ "Non-interned Japanese")
    ) |>
  select(id, RACE, female, BIRTHYR, BPL, p_ij, treat, group,
         age_1940, age_1950,
