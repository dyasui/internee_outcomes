...
** Selecting HISTID's for my sample
...
** MLP Linked to Census Microdata
This function is used to combine IPUMS full-count census data from two different census years together with the MLP linking crosswalks provided in version 2.0.
It takes in a tibble of data in 'long' form meaning that the data object should include all microdata from both census years.
#+begin_src R
link_mlp <- function(data, cw, years = c(1940,1950)) {
