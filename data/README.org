...
#+begin_src R
data_int |>
  count(NHGISST, NHGISCTY, STATENAM, NHGISNAM) |>
  arrange(desc(n)) |>
  head(n=100)
#+end_src

Seems like the counties outside of the west coast are getting double counted in the merge.
It also seems weird that there's at least 5,000 internees who seem to be coming from Virginia.
